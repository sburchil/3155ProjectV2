...
def app():
    ...
    st.write(
        "This page is used to compare graduation percentages per school in the US")
    ...
    school_choice = st.selectbox(
        'Which school would you like to see data for?', df['chronname'])
    school_data = df[df['chronname'] == school_choice]

    school_choice2 = st.selectbox(
        'Select a second school to compare', df['chronname'])
    ...
    with col1:
        st.write("This graph is based on " + school_name + "'s current student count: `" +
                 school_count + "` enrolled at the time of sampling.")
    with col2:
